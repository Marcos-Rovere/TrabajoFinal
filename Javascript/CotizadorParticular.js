const cotizarPlan=()=> {
    ...
    let edad = $("#EdadSocioParticular").val()
    let plan = $("#PlanElegidoParticular").val()
    let estadoCivil = $("#estadoCivilParticular").val()
    let hijos = $("#hijosParticular").val()
    let descuento = $("#descuentoParticular").val()
    

    let divResumen = document.getElementById("resumenParticular")
    let divResultado = document.getElementById("resultadoParticular")

    if (edad === '' || plan === '0' || edad < 0 || edad > 100 || descuento === '0'|| hijos === '0'){
        mostrarError ("#errorAlCompletarParticular","Selecciono o completo mal un dato")
        return
    }

    let cotizacion = {edad,plan,hijos,estadoCivil,descuento};
...
function cotizar(cotizacion){
    const {edad,plan,hijos,estadoCivil,descuento} =cotizacion;
    let  resultado = 4000;
...
$("#calcularParticular").click(cotizarPlan)
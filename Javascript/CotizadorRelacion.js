const cotizarPlanDependencia=()=> {
    ...
    let edad = $("#EdadSocioDependencia").val()
    let plan = $("#PlanElegidoDependencia").val()
    let estadoCivil = $("#estadoCivilDependencia").val()
    let hijos = $("#hijosDependencia").val()
    let descuento = $("#descuentoDependencia").val()
    

    let divResumen = document.getElementById("resumenDependencia")
    let divResultado = document.getElementById("resultadoDependencia")

    if (edad === '' || plan === '0' || edad < 0 || edad > 100 || descuento === '0'|| hijos === '0'|| sueldo === ""){
        mostrarError ("#errorAlCompletarDependencia","Selecciono o completo mal un dato")
        return
    }

    let cotizacion = {edad,plan,hijos,estadoCivil,descuento,sueldo};
...
function cotizar(cotizacion){
    const {edad,plan,hijos,estadoCivil,descuento,sueldo} =cotizacion;
    let  resultado = 3500;
    ...
    resultado = resultado - calcularAporte(sueldo)
    if (resultado < 0){
...
function calcularAporte (sueldo){
    var aporte = sueldo * 0.0689
    return aporte
}
